...
export default class serviceActions {
  handleServiceError (error) {
    return (dispatch, getState) => {
      ...
      switch (error.code) {
        case 401: {
          if (sessionCurrentState && !sessionCurrentState.loggingOut) { // If not loggingOut execute dispatch action.
            if (error.details.url !== 'auth/login') { // For login we don't want to be logOut when 401 found.
